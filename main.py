...
parser.add_argument('--no_makeup', type=str, default=os.path.join('imgs', 'no_makeup', '000020.jpg'), help='path to the no_makeup image')
...
for dirPath, dirNames, fileNames in os.walk("imgs/no_makeup"):
    for f in fileNames:
        no_makeup = os.path.join(dirPath, f)
        ...
        for i in range(len(makeups)):
            ...
            generate_pic = np.ones((256, 512, 3))
            generate_pic[:, :256] = X_img
            generate_pic[:, 256:] = Xs_[0]

            #imsave('result'+str(i)+'.jpg', Xs_[0])
            imsave('data/'+f.split(".")[0]+"_"+str(counter)+'.jpg', generate_pic)
            counter+=1
